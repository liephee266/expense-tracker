...
func (_q *BudgetQuery) QueryUser() *UserQuery {
...
func (_q *BudgetQuery) WithUser(opts ...func(*UserQuery)) *BudgetQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *BudgetQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*Budget, init func(*Budget), assign func(*Budget, *Category)) error {
	...
	for i := range nodes {
		if nodes[i].budget_category == nil {
			continue
		}
		fk := *nodes[i].budget_category
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "budget_category" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *BudgetQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Budget, init func(*Budget), assign func(*Budget, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Budget)
	for i := range nodes {
		if nodes[i].budget_user == nil {
			continue
		}
		fk := *nodes[i].budget_user
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "budget_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
